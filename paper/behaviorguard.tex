...
10-feature ($0.009$) (\texttt{results/stylometric\_linguistic\_eval.json}).
...
\texttt{results/baselines\_matched\_fpr\_holdout.json}).
...
value is $\kappa \approx 0.126$ (\texttt{results/cusum\_kappa\_sweep\_personachat.json}).
...
(\texttt{results/cusum\_kappa\_sweep\_*.json}).
...
Corrected $p$-values are archived in \texttt{results/cusum\_holm\_bonferroni.json}:
...
(\texttt{results/sequential\_ato\_study\_wildchat.json}).
...
machine; acquisition note in \texttt{results/wildchat\_corpus\_acquisition.json})
...
63\% to 35\% (AUC 0.870; \texttt{results/sequential\_ato\_study\_mimicry.json}).
...
\texttt{results/sequential\_ato\_online\_dual\_personachat.json}), quantifying
poisoning risk.
...
\texttt{results/weight\_tuning\_aucpr.json}), with linguistic weight driven to zero.
...
$0.228$ (\texttt{results/external\_positive\_holdout\_eval.json}).
...
(\texttt{results/deployment\_math\_audit.json}): cosine-only profiles (EMA
