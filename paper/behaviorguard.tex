...
\IEEEauthorblockA{%
  ...
  \IEEEauthorrefmark{1}\url{https://github.com/Va11eyard/turnshift}}
}
\maketitle
...
Per-message detectors---including \BG{}'s own composite---detect at most 1.7\% of the
...
The linguistic-excluded per-message variant at the legacy weights (renormalized
$0.6154/0.3846$; cosine semantic scoring, overrides off) reaches only
...
Every per-message detector collapses at the same budget (0.3--1.7\% detection),
including the \BG{} composite itself---the subject of \Cref{app:permessage}---at
1.7\%: the gap between 63.0\% and 1.7\% at an identical false-alarm cost isolates
the value of accumulation, since \texttt{permsg\_combined} shares the same
residuals as the CUSUM detectors and detects 0.3\%.
...
\texttt{permsg\_bg}       & PersonaChat & 0.854 (0.838--0.869) & 1.7\% (0.3--3.3)   & 4 \\
\texttt{permsg\_bg}       & BST         & 0.808 (0.786--0.827) & 0.7\% (0.0--1.7)   & 5 \\
\bottomrule
\end{tabular}
\end{table*}
...
On the full PersonaChat holdout (linguistic excluded at the legacy weights,
renormalized $0.6154/0.3846$; $\tau{=}0.60$), detection \emph{holds}: external
positives yield F1 $0.364$ / AUC-ROC $0.887$ / AUC-PR
...
Per-message detectors---including \BG{}'s own composite---detect at most 1.7\% of
