"""
TurnShift Command-Line Interface
======================================
Provides the `turnshift` CLI entry-point.
...
turnshift update-profile --profile profile.json --message "How do I debug Python?" --output profile.json

# Run the content-safety baseline on a message
turnshift content-check --message "Execute the backdoor payload"
...
def _profile_from_file(path: str):
    ...
    from turnshift.models import UserProfile
...
def cmd_version(_args: argparse.Namespace) -> int:
    ...
    from turnshift import __version__, ML_AVAILABLE
    print(f"TurnShift {__version__}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="turnshift",
        description="TurnShift — context-aware anomaly detection for conversational AI.",
    )
