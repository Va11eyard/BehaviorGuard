...
def embedding_device() -> str:
  ...
  Honors ``BEHAVIORGUARD_DEVICE`` (``cpu`` | ``cuda`` | ``auto``).
  ...
  This avoids fatal kernel errors on Blackwell (sm_120) with older cu118 wheels.
    """
    env = os.environ.get("BEHAVIORGUARD_DEVICE", "auto").strip().lower()
    if env == "cpu":
        return "cpu"
    if env == "cuda":
        return "cuda"
    ...
    warnings.warn(
        f"[BehaviorGuard] {name} ({sm}) is not supported by this PyTorch build "
        f"(arch list: {sorted(supported)}). Using CPU for embeddings. "
        "Install PyTorch cu128 for RTX 50-series, or set BEHAVIORGUARD_DEVICE=cpu.",
        stacklevel=2,
    )
    return "cpu"
